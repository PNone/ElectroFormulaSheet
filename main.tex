...
\begin{itemize}
    ...
    \item \textbf{תאוצה-4:} $A^\mu = \frac{dU^\mu}{d\tau} = \gamma \frac{d}{dt}\left[\gamma(c, \vec{v})\right] = \gamma\left(c \dot{\gamma}, \, \dot{\gamma}\vec{v} + \gamma\vec{a}\right)$
    \\
...
\end{itemize*}
    ...
    \item \textbf{חוק שני של ניוטון (כוח-4):} $K^\mu = \frac{dP^\mu}{d\tau} = m A^\mu = \gamma \left( \frac{\vec{F} \cdot \vec{v}}{c}, \, \vec{F} \right)$
...
\begin{itemize}
    \item \textbf{הגדרה:} $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu \quad | \quad F^{ij} = \epsilon^{ijk} B^k$
...
\begin{itemize}
    ...
    \[
    \partial_\mu T^\mu{}_\nu = -\frac{1}{c^2} F_{\nu\mu} J^\mu, \quad \partial_\mu T^{\mu\nu} = -\frac{1}{c^2} F^{\nu\mu} J_\mu
    \]
...
\section*{טנזור לוי-צ'יוויטה ( $E_{\alpha\beta\gamma\delta} = \text{sgn}(g)\sqrt{|g|}\,\epsilon_{\alpha\beta\gamma\delta}, \ E^{\alpha\beta\gamma\delta} = \frac{1}{\sqrt{|g|}}\,e^{\alpha\beta\gamma\delta}$)}
\begin{itemize}
...
\begin{itemize}
    ...
    $ \\
    \textbf{מעבר לרכיב פיזיקלי (בכיוון $\hat{e}_i$):} $F^{\hat{i}} = h_i F^i = \sqrt{g_{ii}} \, F^i$
...
\begin{itemize}
    ...
    {\small$\begin{pmatrix} \hat{r} \\ \hat{\theta} \\ \hat{\phi} \end{pmatrix} = 
\begin{pmatrix} 
...
\textbf{קיצורים וסימונים:} $t' = t - r/c$, \quad $\vec{E} = \vec{B} \times \hat{r}$, \quad $\vec{U}_{\mathbf{Q}} \equiv \dddot{\mathbf{Q}} \cdot \hat{r}$, $\quad U_r \equiv \hat{r} \cdot \dddot{\mathbf{Q}} \cdot \hat{r}$. כאשר $(\mathbf{Q} \cdot \hat{r})_i = \sum_j Q_{ij} \hat{r}_j$}
