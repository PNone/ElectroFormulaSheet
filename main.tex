...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \end{itemize}
        *(כאשר $+$ מיועד לבעיה חיצונית $r \ge R$, ו-$-$ לפנימית $r \le R$)*
    \end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        \item \textbf{ייצוג נפח רציף מרוכז (צפיפות זרם של מטען נקודתי $q$):}
...
\begin{itemize}
    ...
    $$P_{\text{synch}} = \frac{2 q^2}{3 c^3} \gamma^4 \left( \frac{|q| B \beta_\perp}{\gamma m} \right)^2 = \frac{2 q^4 B^2 \beta_\perp^2 \gamma^2}{3 m^2 c^3} \propto \frac{\gamma^2}{m^2} \overset{E=\text{const}}{\propto} m^{-4}$$
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        ...
        צפיפות המטען מתאפסת בכל המרחב ($\rho = 0$), ולכן המומנט הדיפולי החשמלי מתאפס תמיד *(אין קרינת דיפול חשמלי)*:
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{הגדרת המומנטים האפקטיביים:}
    \begin{enumerate}
        \item \textbf{דיפול חשמלי אפקטיבי ($E1_{\text{eff}}$):} 
        נקבע ישירות ע"י אינטגרל נפחי על הזרם (הסדר האפס בפיתוח מרחבי $r'^0$):
        $$\dot{\vec{p}}_{\text{eff}}(t) \equiv \int_V \vec{J}(\vec{r}', t) \, d^3r' \quad \implies \quad \ddot{\vec{p}}_{\text{eff}}(t) = \frac{\partial}{\partial t} \int_V \vec{J}(\vec{r}', t) \, d^3r'$$
        *אם $\ddot{\vec{p}}_{\text{eff}} \neq 0$, הקרינה המובילה היא מסדר דיפול חשמלי אפקטיבי.*

        \item \textbf{דיפול מגנטי אפקטיבי ($M1_{\text{eff}}$):} 
        החלק האנטי-סימטרי של האיבר בסדר הראשון ($r'^1$):
        $$\vec{m}_{\text{eff}}(t) \equiv \frac{1}{2c} \int_V \left( \vec{r}' \times \vec{J}(\vec{r}', t) \right) d^3r'$$

        \item \textbf{קוודרופול חשמלי אפקטיבי ($E2_{\text{eff}}$):} 
        נגזרת הזמן של טנזור הקוודרופול מבוססת על ההגדרה $Q_{ij} = \frac{1}{2} \int \rho (3x'_i x'_j - r'^2 \delta_{ij}) d^3r'$ בשילוב משוואת הרציפות:
        $$\dot{Q}_{\text{eff}, ij}(t) \equiv \frac{1}{2} \int_V \left( 3 x'_i J_j + 3 x'_j J_i - 2 (\vec{r}' \cdot \vec{J}) \delta_{ij} \right) d^3r'$$
    \end{enumerate}

    \item \textbf{אלגוריתם חישוב מולטיפול מוביל מתוך צפיפות זרם $\vec{J}$:}
\begin{enumerate}[leftmargin=*]
    \item \textbf{שלב 1 — פיתוח פאזה בשדה הרחוק:} 
    מפרקים את הזרם במערכת עבור מקור הקטן בהרבה מאורך הגל ($\frac{\omega L}{c} \ll 1$):
    $$\vec{J}\left(\vec{r}', t_0 + \frac{\hat{n}\cdot\vec{r}'}{c}\right) \approx \vec{J}(\vec{r}', t_0) + \left(\frac{\hat{n}\cdot\vec{r}'}{c}\right) \frac{\partial \vec{J}(\vec{r}', t_0)}{\partial t_0} + \dots$$

    \item \textbf{שלב 2 — בדיקת דיפול חשמלי אפקטיבי ($E1_{\text{eff}}$):} 
    מחשבים את האינטגרל מסדר אפס:
    $$\dot{\vec{p}}_{\text{eff}}(t_0) = \int_V \vec{J}(\vec{r}', t_0) \, d^3r'$$
    * **אם $\ddot{\vec{p}}_{\text{eff}} \neq 0$:** זהו המולטיפול המוביל! הספק הקרינה הממוצע:
      $$\langle P_{E1} \rangle = \frac{2}{3 c^3} \left\langle |\ddot{\vec{p}}_{\text{eff}}(t_0)|^2 \right\rangle$$

    \item \textbf{שלב 3 — בדיקת דיפול מגנטי אפקטיבי ($M1_{\text{eff}}$) וקוודרופול חשמלי אפקטיבי ($E2_{\text{eff}}$):} 
    אם $\ddot{\vec{p}}_{\text{eff}} = 0$, עוברים לאיבר הסדר הראשון ($\frac{\partial \vec{J}}{\partial t_0}$) \underline{ומחשבים את שני המומנטים}:

    \begin{itemize}
        \item \textbf{חישוב $M1_{\text{eff}}$ (דיפול מגנטי):} $\vec{m}_{\text{eff}}(t_0)$.
        אם $\ddot{\vec{m}}_{\text{eff}} \neq 0$, הוא תורם לקרינה. תרומת ההספק הממוצעת של $M1$:
        $$\langle P_{M1} \rangle = \frac{2}{3 c^3} \left\langle |\ddot{\vec{m}}_{\text{eff}}(t_0)|^2 \right\rangle$$


        \item \textbf{חישוב $E2_{\text{eff}}$ (קוודרופול חשמלי):}
        מחשבים את נגזרת טנזור הקוודרופול האפקטיבי - $\dot{Q}_{\text{eff}, ij}(t_0)$.
        אם $\dddot{Q}_{\text{eff}, ij} \neq 0$, הוא תורם לקרינה. תרומת ההספק הממוצעת של $E2$:
        $$\langle P_{E2} \rangle = \frac{1}{45 c^5} \sum_{i,j} \left\langle |\dddot{Q}_{\text{eff}, ij}(t_0)|^2 \right\rangle$$
    \end{itemize}

    \item \textbf{שלב 4 — סיכום והכרעה בסדר השני:}
    * אם **רק אחד** מהם שונה מאפס ($\ddot{\vec{m}}_{\text{eff}} \neq 0$ או $\dddot{\mathbf{Q}}_{\text{eff}} \neq 0$), הוא המולטיפול המוביל.
    * אם **שניהם שונים מאפס**, שתי התרומות מופיעות באותו סדר גודל פיזיקלי ($v/c$), וההספק הכולל בשדה הרחוק הוא סכום שניהם:
      $$\langle P_{\text{total}} \rangle = \langle P_{M1} \rangle + \langle P_{E2} \rangle$$
      *(איברי ההצלבה בין $M1$ ל-$E2$ מתאפסים בעת ביצוע האינטגרציה הזוויתית על פני הנגזרת המרחבית של כל הספירה)*.
