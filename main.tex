...
\DeclareMathSizes{8.5}{8.5}{7}{6} % {fontsize}{textstyle}{scriptstyle}{scriptscriptstyle}
\usepackage{bm}
\usepackage{array}
\usepackage{booktabs}
...
\begin{itemize}
    ...
    \item \textbf{נגזרת (\textenglish{4-Grad}):} $\partial_\mu \equiv \frac{\partial}{\partial x^\mu} = \left(\frac{1}{c}\frac{\partial}{\partial t}, \vec{\nabla}\right)$, \ $\partial^\mu \equiv \frac{\partial}{\partial x_\mu} = \left(\frac{1}{c}\frac{\partial}{\partial t}, -\vec{\nabla}\right)$
...
\begin{itemize}
    ...
    \item \textbf{סיבוב אקטיבי של ו' הבסיס ($\vec{e}'_\mu = \vec{e}_\nu (\Lambda^{-1})^\nu{}_\mu$):}
...
\[
G_D(\vec{r},\vec{r}') = \sum_{\ell=0}^\infty \sum_{m=-\ell}^\ell \frac{4\pi}{2\ell+1} \left[ \frac{r_<^\ell}{r_>^{\ell+1}} - W_\ell(r,r') \right] Y_{\ell m}^*(\theta',\phi') Y_{\ell m}(\theta,\phi)
\]
\[
W_\ell = \begin{cases} 
\dfrac{(rr')^\ell}{R^{2\ell+1}} & \text{בתוך הספירה } (r,r'<R) \\[4pt]
\dfrac{R^{2\ell+1}}{(rr')^{\ell+1}} & \text{מחוץ לספירה } (r,r'>R)
\end{cases}
\]
...
\[
\phi(\vec{r}) = \frac{q}{r} + \frac{\vec{d} \cdot \vec{r}}{r^3} + \frac{1}{2} \sum_{i,j} Q_{ij} \frac{x_i x_j}{r^5} + \mathcal{O}\left(\frac{1}{r^4}\right)
\]
\textbf{אינדקסים:} 
$
\phi(\vec{r}) = \dfrac{q}{r} + \dfrac{d_i x_i}{r^3} + \frac{1}{2} Q_{ij} \dfrac{x_i x_j}{r^5} + \dots \ (i,j \in \{x,y,z\})
$
...
\begin{itemize}
    ...
    $
    Q_{ij} = \int \rho(\vec{r}') \left( 3x'_i x'_j - r'^2 \delta_{ij} \right) d^3r'
    $
\end{itemize}
...
\[
\vec{A}(\vec{r}) = \frac{1}{c} \int \frac{\vec{J}(\vec{r}')}{|\vec{r} - \vec{r}'|} \, d^3r'
\]

\noindent
\textbf{פוטנציאל וקטורי מעוכב (תלוי בזמן, כיול לורנץ $\nabla \cdot \vec{A} + \frac{1}{c}\frac{\partial \phi}{\partial t} = 0$):}
\[
\vec{A}(\vec{r}, t) = \frac{1}{c} \int \frac{\vec{J}\left(\vec{r}', \, t - \frac{|\vec{r} - \vec{r}'|}{c}\right)}{|\vec{r} - \vec{r}'|} \, d^3r'
\]
