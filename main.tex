...
\section*{שטף פוינטינג והספק מוקרן ממטען נק' מואץ כללי (נוסחאות \textenglish{Lienard-W})}

\textbf{התרחיש הפיזיקלי:} קרינה בשדה הרחוק ממטען יחסותי $q$ במסלול שרירותי, בקישור בין זמן הצופה $t$ לזמן המעוכב של $q$: $t' = t - r/c$.

\begin{itemize}[leftmargin=*]
    \item \textbf{שטף פוינטינג בשדה הרחוק בזמן הצופה ($t$):}
    $$\vec{S}(t) = \frac{c}{4\pi} |\vec{E}|^2 \hat{n} = \frac{q^2}{4\pi c^3 \kappa^6 r^2} \left| \hat{r} \times \left[ (\hat{r} - \vec{\beta}_q) \times \vec{a}_q \right] \right|^2 \hat{n} \quad \Big|_{t'=t-r/c}$$
    *(כאשר $\hat{n} = \hat{r}$ הוא כיוון הצופה, ו-$\kappa = 1 - \hat{r} \cdot \vec{\beta}_q$)*.
    ...
    $\frac{dP(t')}{d\Omega} = \kappa |\vec{S}| r^2$
    ...
    $$P(t') = \int \kappa \vec{S} \cdot d\vec{a} = \frac{2}{3} \frac{q^2}{c^3} \gamma^4 \left[ \vec{a}_q^2 + \gamma^2 (\vec{\beta}_q \cdot \vec{a}_q)^2 \right] \quad \Big|_{t'=t-r/c}$$
\end{itemize}
